...
struct LibraryProduct <: AbstractProduct
    ...
    function LibraryProduct(paths::Vector{<:AbstractString},
        ...
        if static !== nothing && static.varname !== nothing
            throw(ArgumentError("Subordinate StaticLibraryProduct of '$(varname)' must not declare its own `varname` (got '$(static.varname)'); the identity comes from the parent LibraryProduct"))
        end
